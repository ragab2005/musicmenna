...
@app.on_message(filters.command(["قران","quran"],""))
async def quran(c,msg):
    ...
    keyboard.append([ikb("", callback_data = "next-1")])
    await msg.reply("• اهلا بك عزيزي في قسم القران تقدر تشغل الان اتبع الازرار ◟", reply_markup = ikm(keyboard))
...
async def show_quran(c,cq):
    ...
    try:
        ...
        await cq.edit_message_reply_markup(ikm([[ikb("II", callback_data = f"ADMIN Pause|{cq.message.chat.id}"),ikb("▷", callback_data=f"ADMIN Resume|{cq.message.chat.id}")],[ikb("▢", callback_data = f"ADMIN Stop|{cq.message.chat.id}")]]))
    except:
